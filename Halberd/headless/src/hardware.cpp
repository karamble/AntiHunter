...
void loadConfiguration() {
    ...
    // already persists to NVS internally; the WiFi block's extra putString is
    // a pre-existing redundant write and should not be propagated here).
...
bool readINA219() {
    ...
    // Re-write CAL before bus-voltage read.
    ok &= ina_write_u16(INA_REG_CALIBRATION, INA_CAL_VALUE);
    // Bus voltage: read twice, use second (Waveshare reference quirk).
    ok &= ina_read_u16(INA_REG_BUS_VOLTAGE, &raw);
    ok &= ina_read_u16(INA_REG_BUS_VOLTAGE, &raw);
    float volts = (raw >> 3) * INA_BUS_VOLTAGE_LSB_V;

    // Re-write CAL before current read (matches gotailme).
    ok &= ina_write_u16(INA_REG_CALIBRATION, INA_CAL_VALUE);
